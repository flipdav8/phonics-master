<template>
  <div
    class="fit flex column items-center justify-center non-selectable phonics-page grid-container"
  >
    ...
    <div v-show="mounted" class="flex flex-center full-width hiddens z-top">
      <!-- UNITS -->
      <div
        v-show="word == null"
...
export default defineComponent({
  ...
  methods: {
    async mountedMethod() {
      ...
      if (this.select_course_id != undefined) {
        this.course_id = this.select_course_id;
      } else if (this.$route.query.course_id != undefined) {
        this.course_id = this.$route.query.course_id;
      }

      if (this.select_student !== undefined) {
    ...
    async setupCourse() {
      ...
      if (this.select_course_id !== undefined) {
        ...
        let course_units = await Courses.getCourseUnits(this.select_course_id);
        ...
        //TODOo: save course to db.. sync all blocks for everyone
        let blocks = (await Courses.getBlocks()).map((e) => ({
          ...e.block,
          ...{ id: e.id },
        }));
